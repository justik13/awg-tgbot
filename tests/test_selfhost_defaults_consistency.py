...
def test_config_defaults_match_selfhost_profile():
    assert DEFAULT_ENV["QOS_ENABLED"] == "1"
    assert DEFAULT_ENV["DEFAULT_KEY_RATE_MBIT"] == "150"
    assert DEFAULT_ENV["QOS_STRICT"] == "0"
...
def test_installer_selfhost_defaults_match_runtime_defaults():
    ...
    assert 'SELFHOST_QOS_ENABLED_DEFAULT="1"' in script
    assert 'SELFHOST_DEFAULT_KEY_RATE_MBIT_DEFAULT="150"' in script
    assert 'SELFHOST_QOS_STRICT_DEFAULT="0"' in script
