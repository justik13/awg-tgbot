...
BROADCAST_RUNNING_STALE_SECONDS = env_int('BROADCAST_RUNNING_STALE_SECONDS', int(DEFAULT_ENV['BROADCAST_RUNNING_STALE_SECONDS']))
...
REFERRAL_RECURRING_INVITER_BONUS_DAYS = env_int(
    'REFERRAL_RECURRING_INVITER_BONUS_DAYS',
    int(DEFAULT_ENV['REFERRAL_RECURRING_INVITER_BONUS_DAYS']),
)
REFERRAL_RECURRING_MIN_PURCHASE_DAYS = env_int(
    'REFERRAL_RECURRING_MIN_PURCHASE_DAYS',
    int(DEFAULT_ENV['REFERRAL_RECURRING_MIN_PURCHASE_DAYS']),
)
...
# Может быть изменён при runtime если порт занят (см. app.py)
NODE_API_PORT = env_int('NODE_API_PORT', 8444)

def update_node_api_port(new_port: int) -> None:
    """Обновляет NODE_API_PORT в runtime (используется при автоподборе порта)."""
    global NODE_API_PORT
    NODE_API_PORT = new_port
